...
describe("user entry text normalization", () => {
  it("converts mixed and uppercase human text to proper case", () => {
    expect(properCaseUserText("  mACHINE   mASTER loCATION ")).toBe(
      "Machine Master Location"
    )
    expect(properCaseUserText("PRE-SETTING / FINAL CHECK")).toBe(
      "Pre-Setting / Final Check"
    )
    expect(properCaseUserText("O'NEIL TOOL ROOM")).toBe("O'Neil Tool Room")
    expect(properCaseUserText("DON'T CHANGE OPERATOR'S CODE")).toBe(
      "Don't Change Operator's Code"
    )
    expect(properCaseUserText("HOD")).toBe("HOD")
  })
