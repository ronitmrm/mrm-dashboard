...
const protectedFieldFragment = /(?:base64|email|file|href|mobile|password|path|phone|secret|token|uri|url)/;
const isoDateOrTime = /^(?:\d{4}-\d{2}-\d{2}(?:[T ][0-9T:.+Z-]*)?|\d{1,2}:\d{2}(?::\d{2})?)$/i;
...
export function normalizeUserEnteredPayload<T>(value: T, fieldName = ""): T {
  ...
  if (Array.isArray(value)) {
    return value.map((item) => normalizeUserEnteredPayload(item, fieldName)) as T;
  }
  if (typeof value === "object" && value !== null) {
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [key, normalizeUserEnteredPayload(item, key)]),
    ) as T;
  }
  return value;
}
...
export function preservesUserEnteredTextCase(fieldName: string) {
  ...
  return preservedFieldNames.has(normalizedFieldName)
...
function normalizeUserEnteredString(value: string, fieldName: string) {
  if (fieldName.toLowerCase().replace(/[^a-z0-9]/g, "") === "filebase64") return value;
  const cleaned = value.trim();
  if (!cleaned || preservesUserEnteredTextCase(fieldName) || isoDateOrTime.test(cleaned) || emailOrUrl.test(cleaned)) {
    return cleaned;
  }
