...
export default async function CustomerRevisionCostingPage({
  params,
}: {
  ...
  const { revisionId: selectedRevisionId } = await params
  if (
  ...
  const data = await (async () => {
    try {
      const selectedRevision = await repository.getCustomerBulkPriceRevision(
        "MRMPL",
        selectedRevisionId
      )
      if (!selectedRevision) notFound()
      if (selectedRevision.status === "Completed")
        redirect(
          `/commercial/customer-bulk-revision/history/${encodeURIComponent(selectedRevisionId)}`
        )
      const [stages, prices] = await Promise.all([
      ...
      return { selectedRevision, stages, prices }
    } finally {
  ...
  return (
    <div className="grid gap-6">
      ...
      />
      <SectionCard id="customer-bulk-workbench">
        <CardHeader>
              ...
              {!isCompleted ? (
                ...
                >
                  ...
                  <OperationalTable
                    ...
                    excelFilters
                    filterStorageKey={`customer-bulk-revision:${selectedRevision.id}`}
                    filteredSelection={{
