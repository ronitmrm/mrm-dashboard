...
import { customerCostingTaskHref } from "@/lib/pricing/costing-task-routes"
...
export async function CustomerParameterCostingView({
  searchParams,
  standalone = false,
}: {
                      ...
                      >
                        ...
                        <TableCell>
                          <Button asChild size="sm" variant="outline">
                            <Link href={customerCostingTaskHref(task)}>
                              {actionLabel(task.taskType)}
                            </Link>
                          </Button>
                        </TableCell>
                      </TableRow>
          ...
          <CardContent>
            {selectedTask ? (
              <form action={saveQuoteAction} className="grid gap-6">
                ...
                {selectedQueueTask?.targetPrice !== null &&
                selectedQueueTask?.targetPrice !== undefined ? (
                ...
                <FieldSet>
                  ...
                  <FieldGroup>
                    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                      <Field>
                        <FieldLabel htmlFor="customer_part_code">
                          Customer Part Code
                        </FieldLabel>
                        <Input
                      ...
                      <NumberField
                        defaultValue={percent(
                          selectedTask.quoteDefaults.profitPercent
                        )}
                        label="Profit (%)"
                        name="profit_percent"
                      />
                      {isRootParent || isDirectPurchase ? (
                        ...
                        <>
                          <NumberField
                            defaultValue={selectedTask.quoteDefaults.scrapRate}
                            label="Scrap INR / Kg"
                            name="scrap_rate"
                          />
                          <NumberField
                            defaultValue={
                              selectedTask.quoteDefaults.purchaseTimes
                            }
                      ...
                      <Field>
                        ...
                        <Input
                          defaultValue={
                            selectedTask.quoteDefaults.packaging ?? ""
                          }
                          id="packaging"
                          name="packaging"
                        />
                      ...
                      <Field>
                        ...
                        <Input
                          defaultValue={
                            selectedTask.quoteDefaults.shippingTerms ?? ""
                          }
                          id="shipping_terms"
                          name="shipping_terms"
                        />
                ...
                {leafItems.length ? (
                  <FieldSet>
                    ...
                    <FieldGroup>
                      {leafItems.map((item) => (
                        <div
                          className="grid gap-4 rounded-2xl border p-4 sm:grid-cols-4"
                          key={item.itemId}
                        >
                          <input
                            name="child_item_id"
                          ...
                          {item.pricingMethod === "Direct Purchase" ? (
                            ...
                            <>
                              <NumberField
                                defaultValue={
                                  componentDefaults.get(item.itemId)
                                    ?.scrapRate ?? 0
                                }
                                label="Scrap INR / Kg"
                                name="child_scrap_rate"
            ...
            {selectedTask ? (
              <form
                action={sendQuoteBackToProductCostingAction}
                className="mt-6 border-t pt-6"
              >
                <input
                  name="enquiry_id"
                  type="hidden"
                  value={selectedTask.enquiryId}
                />
                <input
                  name="item_id"
                  type="hidden"
