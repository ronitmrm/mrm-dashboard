...
import {
  productionGrossWeightKg,
  productionPieceWeightGrams,
} from "@/lib/production-session-entry"
import {
  carriedDowntimeRows,
...
const outputPending = (row: Row) => row.outputPending === true
...
function CarriedDowntimeTable({ rows: carriedRows, sessions, options, onSelect, onAction }: {
 ...
 return <SectionCard><CardHeader><CardTitle>Unresolved downtime</CardTitle><p className="text-sm text-muted-foreground">Problems carried from a previous shift. Off-shift hours are not counted.</p></CardHeader><CardContent><ScrollableTable headers={["Machine", "Reason", "Interval", "Status", "Action"]}>{carriedRows.map((row) => {
    ...
    return <TableRow key={`${row.machineNumber}-${row.eventId}`}><TableCell className="font-medium">{row.machineNumber}</TableCell><TableCell><div>{row.reasonCode || "Downtime"}</div><div className="text-xs text-muted-foreground">{row.reasonName || "Unresolved machine problem"}</div></TableCell><TableCell className="whitespace-nowrap text-xs">{row.state === "open" ? `Started ${formatDateTime(row.startedAt)}` : `Closed ${formatDateTime(row.endedAt)}`}</TableCell><TableCell><StatusBadge tone={row.breakdownLinked ? "danger" : row.state === "open" ? "information" : "warning"} value={row.breakdownLinked ? "Managed in Maintenance" : row.state === "open" ? "Current downtime" : "Carried unresolved"} /></TableCell><TableCell><div className="flex flex-wrap gap-2">{row.breakdownLinked ? "-" : row.state === "open" && currentSession ? <Button className="h-9" variant="destructive" onClick={() => onAction("downtimeEnd", currentSession)}>Close downtime</Button> : sameSession ? <Button className="h-9" onClick={() => onAction("end", currentSession)}>End session</Button> : currentSession ? <Button className="h-9" onClick={() => onAction("downtime", { ...currentSession, carriedReasonCode: row.reasonCode, carriedReasonName: row.reasonName, carriedStartAt: currentSession.startedAt })}>Continue this shift</Button> : <Button className="h-9" disabled={!canSelect} onClick={() => onSelect(row.machineNumber)}>Select machine</Button>}{row.state === "carried" && !row.breakdownLinked ? <Button className="h-9" variant="outline" onClick={() => onAction("carryResolve", carryTarget)}>Mark resolved</Button> : null}</div></TableCell></TableRow>
  })}</ScrollableTable></CardContent></SectionCard>
}
...
function ActionSheet({ action, target, floor, shift, employees, control, saving, message, onOpenChange, onSave }: { action: Action | null; target: Row | null; floor: ProductionFloorCode; shift: ReturnType<typeof productionShiftAt>; employees: Array<{ code: string; name: string }>; control: Row; saving: boolean; message: string; onOpenChange: (open: boolean) => void; onSave: (entryType: string, payload: Row) => void }) {
  ...
  const [startAt, setStartAt] = useState(
    action === "lateDowntime" && text(target?.startedAt)
      ? istDateTimeInputValue(text(target?.startedAt))
      : text(target?.carriedStartAt)
      ? istDateTimeInputValue(text(target?.carriedStartAt))
      : defaults.startAt
  )
  const [endAt, setEndAt] = useState(
    (action === "correctClose" || action === "lateDowntime") && text(target?.endedAt)
      ? istDateTimeInputValue(text(target?.endedAt))
      : action === "downtimeEnd" || action === "carryResolve"
      ? actualNow
      : defaults.endAt
  )
  const [startCount, setStartCount] = useState("")
  const [endCount, setEndCount] = useState(action === "correctClose" ? text(target?.endCount) : "")
  const [producedKg, setProducedKg] = useState(action === "correctClose" ? text(target?.netWeightKg) : "")
  const [crates, setCrates] = useState(action === "correctClose" ? text(target?.crateCount) : "")
  const [endReason, setEndReason] = useState<string>(action === "correctClose" ? text(target?.endReason) || defaults.endReason : defaults.endReason)
  ...
  const hasProducedWeight = producedKg !== ""
  const hasCrateCount = crates !== ""
  const grossWeightKg = method === "weight" && hasProducedWeight
    ? productionGrossWeightKg({
        crateCount: number(crates),
        crateWeightKg: 2,
        producedWeightKg: number(producedKg),
      })
    : undefined
  const submit = () => {
    ...
    if (action === "end") onSave("production_session_close", { sessionId, enteredRole: role, endedAt: endAtIso, endReason, endCount: method === "counter" ? number(endCount) : undefined, grossWeightKg, crateCount: method === "weight" && hasCrateCount ? number(crates) : undefined, crateWeightKg: method === "weight" && hasProducedWeight ? 2 : undefined })
    if (action === "correctClose") onSave("production_session_correct", { sessionId, correctionReason, enteredRole: role, endedAt: endAtIso, endReason, endCount: method === "counter" ? number(endCount) : undefined, grossWeightKg, crateCount: method === "weight" ? number(crates) : undefined, crateWeightKg: method === "weight" ? 2 : undefined })
  ...
  const weightCloseIsValid = (!hasProducedWeight && !hasCrateCount) || (hasProducedWeight && hasCrateCount)
  const valid = action === "start" ? Boolean(operator && startAtIso && method && pieceWeight > 0 && (method === "weight" || startCount || text(plan.carriedStartCount)))
    ...
    : action === "correctClose" ? Boolean(correctionReason && endAtIso && endReason && (method === "counter" ? endCount : hasProducedWeight && hasCrateCount))
...
function DetailSheet({ session, events, floor, now, onOpenChange, onAction }: { session: Row | null; events: Row[]; floor: ProductionFloorCode; now: Date; onOpenChange: (open: boolean) => void; onAction: (action: Action, row: Row) => void }) {
  ...
  const summary: Array<[string, string | number]> = [
    ...
    ["Efficiency", pending ? "Pending" : efficiency === null ? "-" : `${percent.format(efficiency * 100)}%`],
    ["Rejected", number(session.rejectedPieces)],
    ["Good", pending ? "Pending" : number(session.goodPieces)],
    ...(measurementMethod === "counter"
      ...
      : [
          ["Weight produced", pending ? "Pending" : session.netWeightKg === null ? "-" : `${number(session.netWeightKg)} kg`],
