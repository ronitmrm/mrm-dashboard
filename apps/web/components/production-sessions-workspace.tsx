...
function StartSessionLookup({ options, selected, shift, floor, now, onSelect, onAction, onDetail }: { options: ProductionSessionMachineOption[]; selected?: ProductionSessionMachineOption; shift: ReturnType<typeof productionShiftAt>; floor: ProductionFloorCode; now: Date; onSelect: (machineNumber: string) => void; onAction: (action: Action, row: Row) => void; onDetail: (row: Row) => void }) {
  ...
  return <div className="mx-auto grid w-full max-w-4xl justify-items-center gap-4">
    <div className="w-full max-w-sm"><Field label="Machine number"><NativeSelect value={selected?.machineNumber ?? ""} onChange={(event) => onSelect(event.target.value)}><NativeSelectOption value="">Select running machine</NativeSelectOption>{options.map((option) => <NativeSelectOption key={option.machineNumber} value={option.machineNumber}>{option.machineNumber} · {part(option.plan) || "No planned part"} · JC {job(option.plan) || "-"}</NativeSelectOption>)}</NativeSelect></Field></div>
    {!selected ? <div className="w-full rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground">{options.length ? "Select a running machine to fetch its current planning information." : "No machines are currently running in this Production Unit."}</div> : null}
    {selected && plan ? <div className="w-full rounded-lg border bg-background p-4 sm:p-5">
...
function ActionSheet({ action, target, floor, shift, employees, control, saving, message, onOpenChange, onSave }: { action: Action | null; target: Row | null; floor: ProductionFloorCode; shift: ReturnType<typeof productionShiftAt>; employees: Array<{ code: string; name: string }>; control: Row; saving: boolean; message: string; onOpenChange: (open: boolean) => void; onSave: (entryType: string, payload: Row) => void }) {
  ...
  const [grossKg, setGrossKg] = useState(action === "correctClose" ? text(target?.grossWeightKg) : "")
  const [crates, setCrates] = useState(action === "correctClose" ? text(target?.crateCount) : "")
  const storedCrateWeight = text(target?.crateWeightKg)
  const [crateWeightKg, setCrateWeightKg] = useState(
    action === "correctClose" && productionCrateWeightOptions.some((weight) => String(weight) === storedCrateWeight)
      ? storedCrateWeight
      : ""
  )
  ...
  const summary: Array<[string, string | number]> = [
    ...
    ...(measurementMethod === "counter"
      ...
      : [
          ...
          ["Crate weight", session.crateWeightKg === null ? "-" : `${number(session.crateWeightKg)} kg`],
        ] as Array<[string, string | number]>),
  ]
