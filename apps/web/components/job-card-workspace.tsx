...
export function JobCardWorkspace({ floor, jobCardNumber }: { floor: ProductionFloorCode; jobCardNumber: string }) {
  ...
  return <main className="grid gap-4 p-4 md:p-6">
    ...
    {workspace ? <>
      ...
      {activeTab === "masters" ? <Card><CardHeader><CardTitle>Selected Master Data</CardTitle></CardHeader><CardContent className="grid gap-5">
      ...
      {activeTab === "production" ? <section className="grid gap-4">
        ...
        <Card><CardHeader><CardTitle>Planner Movement History</CardTitle></CardHeader><CardContent><div className="overflow-x-auto rounded-md border"><Table excelFilters><TableHeader><TableRow><TableHead>Decision Time</TableHead><TableHead>Action</TableHead><TableHead>Setup</TableHead><TableHead>Machine Movement</TableHead><TableHead>Reason</TableHead><TableHead>Session Settlement</TableHead><TableHead className="text-right">Good Output</TableHead><TableHead>Planner</TableHead></TableRow></TableHeader><TableBody>{plannerMovements.length ? plannerMovements.map((row, index) => {
          ...
          return <TableRow key={`${text(row.decisionId)}-${text(row.actionType)}-${index}`}><TableCell className="whitespace-nowrap">{date(row.eventTime, true)}</TableCell><TableCell><Badge variant="outline">{display(row.actionLabel)}</Badge></TableCell><TableCell>{display(row.setupNumber)}</TableCell><TableCell className="font-medium">{movement}</TableCell><TableCell className="min-w-48">{display(row.reason)}</TableCell><TableCell className="min-w-52">{references.length ? <>{references.map((reference) => <span className="block font-mono text-xs" key={reference}>{reference}</span>)}{row.settledAt ? <span className="block text-xs text-muted-foreground">Settled {date(row.settledAt, true)}</span> : null}</> : row.downtimeOpen && row.activeSessionReference ? <><span className="block font-mono text-xs">{display(row.activeSessionReference)}</span><span className="block text-xs text-amber-700 dark:text-amber-300">Downtime remains open</span></> : <span className="text-muted-foreground">No running session affected</span>}</TableCell><TableCell className="text-right font-medium tabular-nums">{references.length ? `${quantity(row.settledGoodPieces)} pcs` : "-"}</TableCell><TableCell>{display(row.recordedByName)}</TableCell></TableRow>
        }) : <TableRow><TableCell colSpan={8} className="py-10 text-center text-muted-foreground">No planner movement recorded.</TableCell></TableRow>}</TableBody></Table></div></CardContent></Card>
        <Card><CardHeader><CardTitle>Production Plan</CardTitle></CardHeader><CardContent><div className="overflow-x-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>Machine</TableHead><TableHead>Setup</TableHead><TableHead>Start</TableHead><TableHead>End</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{planRows.length ? planRows.map((row, index) => <TableRow key={`${value(row, "machineNo", "machineNumber")}-${index}`}><TableCell>{value(row, "machineNo", "machineNumber") || "-"}</TableCell><TableCell>{value(row, "setupNo", "setupNumber") || "-"}</TableCell><TableCell>{date(value(row, "plannedProductionStartDate", "productionStartDate"))}</TableCell><TableCell>{date(value(row, "plannedProductionEndDate", "productionEndDate"))}</TableCell><TableCell>{title(value(row, "runningStatus", "status", "shopFloorStage"))}</TableCell></TableRow>) : <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">No plan rows available.</TableCell></TableRow>}</TableBody></Table></div></CardContent></Card>
      </section> : null}

      {activeTab === "rejection" ? <section className="grid gap-4"><div className="grid gap-3 sm:grid-cols-3"><Metric label="Rejected Pieces" value={quantity(analytics.rejectedPieces)} /><Metric label="Rejection Rate" value={`${quantity(analytics.rejectionPercent, 2)}%`} /><Metric label="Rejection Entries" value={quantity(rejectionEvents.length)} /></div><Card><CardHeader><CardTitle className="flex items-center gap-2"><ShieldAlert className="size-4" /> Rejection Pattern</CardTitle></CardHeader><CardContent><PatternBars emptyText="No rejection recorded." rows={rejectionPatterns} valueKey="quantity" /></CardContent></Card><Card><CardHeader><CardTitle>Rejection Log</CardTitle></CardHeader><CardContent><EventTable emptyText="No rejection entries recorded." rows={rejectionEvents} /></CardContent></Card></section> : null}
      ...
      {activeTab === "log" ? <Card><CardHeader><CardTitle className="flex items-center gap-2"><History className="size-4" /> Complete Job Card Log</CardTitle></CardHeader><CardContent><EventTable emptyText="No Job Card events recorded." rows={events} /></CardContent></Card> : null}
    </> : null}
  </main>
}
