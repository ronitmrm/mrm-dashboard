...
export function JobCardWorkspace({ floor, jobCardNumber }: { floor: ProductionFloorCode; jobCardNumber: string }) {
  ...
  const plannerMovements = workspace?.plannerMovements ?? emptyRows
 ...
 <SectionCard><CardHeader><CardTitle>Planner Movement History</CardTitle></CardHeader><CardContent><div className="overflow-x-auto rounded-md border"><OperationalTable excelFilters><TableHeader><TableRow><TableHead>Decision Time</TableHead><TableHead>Action</TableHead><TableHead>Setup</TableHead><TableHead>Machine Movement</TableHead><TableHead>Reason</TableHead><TableHead>Session Settlement</TableHead><TableHead className="text-right">Good Output</TableHead><TableHead>Planner</TableHead></TableRow></TableHeader><TableBody>{plannerMovements.length ? plannerMovements.map((row, index) => {
          ...
          const toMachine = text(row.toMachineNumber)
          const movement = fromMachine && toMachine ? `${fromMachine} → ${toMachine}` : fromMachine || toMachine || "Planning queue"
 return <TableRow key={`${text(row.decisionId)}-${text(row.actionType)}-${index}`}><TableCell className="whitespace-nowrap">{date(row.eventTime, true)}</TableCell><TableCell><Badge variant="outline">{display(row.actionLabel)}</Badge></TableCell><TableCell>{display(row.setupNumber)}</TableCell><TableCell className="font-medium">{movement}</TableCell><TableCell className="min-w-48">{display(row.reason)}</TableCell><TableCell className="min-w-52">{references.length ? <>{references.map((reference) => <span className="block font-mono text-xs" key={reference}>{reference}</span>)}{row.settledAt ? <span className="block text-xs text-muted-foreground">Settled {date(row.settledAt, true)}</span> : null}</> : row.downtimeOpen && row.activeSessionReference ? <><span className="block font-mono text-xs">{display(row.activeSessionReference)}</span><span className="block text-xs text-[var(--color-warning-text)] ">Downtime remains open</span></> : <span className="text-muted-foreground">No running session affected</span>}</TableCell><TableCell className="text-right font-medium tabular-nums">{references.length ? `${quantity(row.settledGoodPieces)} pcs` : "-"}</TableCell><TableCell>{display(row.recordedByName)}</TableCell></TableRow>
 }) : <TableRow><TableCell colSpan={8} className="py-10 text-center text-muted-foreground">No planner movement recorded.</TableCell></TableRow>}</TableBody></OperationalTable></div></CardContent></SectionCard>
 <SectionCard><CardHeader><CardTitle>Production Plan</CardTitle></CardHeader><CardContent><div className="overflow-x-auto rounded-md border"><OperationalTable><TableHeader><TableRow><TableHead>Machine</TableHead><TableHead>Setup</TableHead><TableHead>Start</TableHead><TableHead>End</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{planRows.length ? planRows.map((row, index) => <TableRow key={`${value(row, "machineNo", "machineNumber")}-${index}`}><TableCell>{value(row, "machineNo", "machineNumber") || "-"}</TableCell><TableCell>{value(row, "setupNo", "setupNumber") || "-"}</TableCell><TableCell>{date(value(row, "plannedProductionStartDate", "productionStartDate"))}</TableCell><TableCell>{date(value(row, "plannedProductionEndDate", "productionEndDate"))}</TableCell><TableCell>{title(value(row, "runningStatus", "status", "shopFloorStage"))}</TableCell></TableRow>) : <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">No plan rows available.</TableCell></TableRow>}</TableBody></OperationalTable></div></CardContent></SectionCard>
      </section> : null}
