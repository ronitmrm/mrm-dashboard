  ...
  return <main className="grid gap-4 p-4 md:p-6">
    ...
    {workspace ? <>
      ...
      {activeTab === "overview" ? <section className="grid gap-4">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Production Progress" value={`${quantity(completion, 1)}%`} note={`${quantity(analytics.actualGoodPieces)} / ${quantity(analytics.orderedQuantity)} good pcs`} /><Metric label="Current Stage" value={currentStage} /><Metric label="Delivery Rating" value={display(delivery.rating)} note={display(delivery.status)} /><Metric label="Order Short" value={quantity(material.orderShortPieces)} note="good pieces still required" /></div>
        <Card><CardHeader><CardTitle>Job Card Progress</CardTitle><CardDescription>Progress is based only on good pieces produced against the ordered quantity.</CardDescription></CardHeader><CardContent className="grid gap-4"><div><div className="mb-1 flex justify-between text-sm"><span>Good production</span><strong className="tabular-nums">{quantity(analytics.actualGoodPieces)} / {quantity(analytics.orderedQuantity)} pcs</strong></div><div className="h-4 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-emerald-600" style={{ width: `${completion}%` }} /></div></div><dl className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4"><Field label="Planned Start" value={date(analytics.plannedStartDate)} /><Field label="Actual Start" value={date(analytics.actualStartAt, true)} /><Field label="Planned End" value={date(analytics.plannedEndDate)} /><Field label="Target Delivery" value={date(delivery.targetDate)} /></dl></CardContent></Card>
      ...
      {activeTab === "production" ? <section className="grid gap-4">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Good Produced" value={quantity(analytics.actualGoodPieces)} note={`${quantity(completion, 1)}% of order`} /><Metric label="Total Produced" value={quantity(analytics.actualProducedPieces)} note="includes rejection" /><Metric label="Runtime" value={`${quantity(analytics.runtimeMinutes)} min`} /><Metric label="Sessions" value={quantity(analytics.sessionCount)} /></div>
