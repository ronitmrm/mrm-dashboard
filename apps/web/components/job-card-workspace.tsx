...
export function JobCardWorkspace({ floor, jobCardNumber }: { floor: ProductionFloorCode; jobCardNumber: string }) {
 ...
 {activeTab === "masters" ? <SectionCard><CardHeader><CardTitle>Selected Master Data</CardTitle></CardHeader><CardContent className="grid gap-5">
        <dl className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><Field label="Part Code" value={jobCard.partCode} /><Field label="Description" value={jobCard.description} /><Field label="Work Order" value={jobCard.workOrderNumber} /><Field label="Selected Route" value={selectedRoute ? `${display(selectedRoute.routeCode)} · ${display(selectedRoute.name)}` : "Planner selection required"} /><Field label="Ordered Quantity" value={quantity(jobCard.orderedQuantity)} /><Field label="Production Type" value={jobCard.productionType} /><Field label="Casting" value={jobCard.casting} /><Field label="Material Grade" value={jobCard.materialGrade} /><Field label="Rod Type / Size" value={`${display(jobCard.rodType)} / ${display(jobCard.rodSize)}`} /><Field label="One-Piece Weight" value={jobCard.weight100Pieces} /><Field label="Pieces / Kg" value={jobCard.piecesPerKg} /><Field label="Product Delivery Default" value={deliveryTarget.productDefaultWorkingDays ? `${quantity(deliveryTarget.productDefaultWorkingDays)} working days` : "Not set"} /></dl>
