...
const defaultWorkbook = path.resolve(
  appDir,
  "../../..",
  "Advanced_Employee_Performance_System.xlsx"
)
...
function parseWorkbook(filePath) {
  ...
  const dataEntries = [
    ...parseRawSheetEntries(
      workbook,
      "machine_master",
      "Machine_Master",
      (row) => text(rowValue(row, "M/C NO", "MACHINE NO", "MACHINE NUMBER"))
    ),
    ...parseRawSheetEntries(
      workbook,
      "dispatch",
      "Main_Floor_Dispatch",
      (row) =>
        text(rowValue(row, "JC NO.", "JC NO", "JOB CARD NO.", "JobCardNo"))
    ),
    ...parseRawSheetEntries(
      workbook,
      "rejection_classification",
      "Rejection Classification",
      (row) => text(rowValue(row, "CODE", "Rejection Reason"))
    ),
    ...parseRawSheetEntries(
      workbook,
      "raw_material_plan",
      "Raw_Material_Date_Plan",
      (row) =>
        text(
          rowValue(row, "RM Date Plan ID", "Production Job Card No.", "PART NO")
        )
    ),
    ...parseRawSheetEntries(
      workbook,
      ...
      "FG_Machine_Planning",
      (row) =>
        text(rowValue(row, "Plan ID", "Production Job Card No.", "PART NO"))
    ),
    ...parseRawSheetEntries(
      workbook,
      "quality_inspection",
      "Quality_Inspection",
      (row) => text(rowValue(row, "Inspection ID", "PART NO"))
    ),
    ...parseDataEntries(workbook, "route", "Planning_Route_Master", routeMap),
    ...parseDataEntries(
      workbook,
      "cycle",
      "Planning_Cycle_Time_Master",
      cycleMap
    ),
    ...parseDataEntries(
      workbook,
      "tooling",
      "Planning_Tooling_Master",
      toolingMap
    ),
    ...parseDataEntries(
      workbook,
...
function parseEntryTypeWorkbook(filePath, entryType) {
  ...
  if (!parser) {
    throw new Error(
      `Unsupported --entry-type "${entryType}". Supported values: ${Object.keys(dataEntryParsers).join(", ")}`
    )
  }
...
const dataEntryParsers = {
  machine_master: (workbook) =>
    parseRawSheetEntries(workbook, "machine_master", "Machine_Master", (row) =>
      text(rowValue(row, "M/C NO", "MACHINE NO", "MACHINE NUMBER"))
    ),
  dispatch: (workbook) =>
    parseRawSheetEntries(workbook, "dispatch", "Main_Floor_Dispatch", (row) =>
      text(rowValue(row, "JC NO.", "JC NO", "JOB CARD NO.", "JobCardNo"))
    ),
  rejection_classification: (workbook) =>
    parseRawSheetEntries(
      workbook,
      "rejection_classification",
      "Rejection Classification",
...
function parseRmInwardEntries(workbook) {
  return rows(workbook, "Work_Order_Import")
    ...
    .filter(
      (payload) =>
        text(payload.jcNo) &&
        (text(payload.rmInwardDate) || text(payload.rmInwardKg))
    )
  ...
  return rows(workbook, "Planner_Route_Selection_Log")
    .map((row) =>
      dropUndefined({
        ...
        optionNumber: text(
          rowValue(row, "SELECTED ROUTE OPTION", "OPTION NUMBER")
        ),
  ...
  return rows(workbook, "Planner_Machine_Constraints")
    .map((row) =>
      dropUndefined({
        ...
        reason:
          text(rowValue(row, "REASON")) ||
          text(rowValue(row, "STATUS")) ||
          "Imported machine constraint",
  ...
  return rows(workbook, "Planner_Route_Changes")
    .map((row) =>
      dropUndefined({
        ...
        changeAfterSetup:
          text(rowValue(row, "CHANGE AFTER SETUP")) || undefined,
  ...
  return rows(workbook, "Setup_Completion_Log")
    .map((row) =>
      dropUndefined({
        ...
        setupNo: text(rowValue(row, "SETUP NO.", "SETUP NO")) || undefined,
        machine:
          text(rowValue(row, "MACHINE", "M/C NO", "MACHINE NO")) || undefined,
        completedBy:
          text(rowValue(row, "COMPLETED BY", "UPDATED BY")) || "Imported",
        remark: text(rowValue(row, "REMARK", "REMARKS")) || undefined,
        createdAt:
          isoDateTime(rowValue(row, "LOGGED ON", "UPDATED DATE")) || undefined,
      })
    )
    .filter((row) => row.jcNo)
}
...
function countPayload(payload) {
  return Object.fromEntries(
    Object.entries(payload).map(([key, rowsToCount]) => [
      key,
      rowsToCount.length,
    ])
  )
...
function writeTableImportFiles(payload) {
  const outDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "mrmpl-workbook-import-")
  )
  const files = {}
...
async function main() {
  ...
  const parsed = entryTypeFilter
    ? parseEntryTypeWorkbook(workbookPath, entryTypeFilter)
    : parseWorkbook(workbookPath)
  ...
  if (!entryTypeFilter && !includePlannerActions)
    console.log(
      "Planner action/decision tables are skipped. Pass --include-planner-actions to import them as data."
    )
  console.table(counts)
  if (parsed.dataEntries)
  ...
  if (
    ...
    throw new Error(
      "CONVEX_SELF_HOSTED_ADMIN_KEY is required when importing into a self-hosted Convex backend."
    )
  }
  ...
  if (
    ...
    throw new Error(
      "Convex deployment env is required. Set CONVEX_DEPLOYMENT/NEXT_PUBLIC_CONVEX_URL for cloud or CONVEX_SELF_HOSTED_URL/CONVEX_SELF_HOSTED_ADMIN_KEY for local self-hosted Convex in apps/web/.env.local."
    )
  }
  ...
  console.log(
    `${replace ? "Replacing" : "Appending to"} ${entryTypeFilter ? "selected entry type" : "workbook tables"} in the selected Convex deployment.`
  )
