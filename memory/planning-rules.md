...
- Shop Floor Tasks also has a bulk downtime entry for all currently running machines. Bulk downtime uses defect/downtime codes from `rejection_reason_master`, writes downtime-only `production_card` rows, and must not write `software_raw`.
- Quality Control Tasks records downtime and rejection for the selected running machine. Downtime uses defect/downtime codes from `rejection_reason_master`. Rejection uses `rejection_type_master`, `rejection_reason_master`, and `rejection_remark_master` codes, with built-in defaults for T1-T4, D1-D58, and R1-R7; QC enters rejected pcs for the selected machine.
- Machinist Tasks records downtime only: machine number, item/setup summary tile, downtime code selected from `rejection_reason_master`, 24-hour downtime start/end (`HH:mm`), and calculated downtime minutes.
