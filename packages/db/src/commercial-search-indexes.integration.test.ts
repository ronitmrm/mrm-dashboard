...
test("10,000-row Sales candidate search uses bounded trigram plans", async () => {
...
test("10,000-row catalog searches retain their declared access paths", async () => {
  const expectedIndexes = [
  ...
  const decoyOrganizationId = organizationByCode.get(`CATALOG-DECOY-${suffix}`)!
