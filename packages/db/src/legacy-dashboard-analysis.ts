...
export function buildLegacyDashboardSnapshot(input: LegacyDashboardInput) {
  ...
  if (
    ...
    maintenanceChecklistMasterRows.length ||
...
function buildProductionControl({
  ...
  for (const row of productionRows) {
    ...
    if (setupKey) {
      const setupRec = getOrCreate(rawBySetup, setupKey, (): PlanningProductionActual => ({ startDate: "", latestDate: "", outputQty: 0, actualQty: 0, rows: 0, dates: new Set<string>() }));
    ...
    if (setupAnyMachineKey) {
      const setupRec = getOrCreate(rawBySetupAnyMachine, setupAnyMachineKey, (): PlanningProductionActual & { machines: Set<string> } => ({ startDate: "", latestDate: "", outputQty: 0, actualQty: 0, rows: 0, dates: new Set<string>(), machines: new Set<string>() }));
      if (prodDate && (!setupRec.startDate || prodDate < setupRec.startDate)) setupRec.startDate = prodDate;
      if (prodDate && (!setupRec.latestDate || prodDate > setupRec.latestDate)) setupRec.latestDate = prodDate;
      if (prodDate) setupRec.dates.add(prodDate);
      setupRec.outputQty += safeNumber(rowValue(row, "PRODUCTION QTY (PCS)", "PROD QTY IN PCS"));
  ...
  const workOrderOutputRows = workOrderRows.map((row) => {
    ...
    const rmInwardKg = safeNumber(rowValue(rmInward ?? {}, "RM INWARD KG.", "rmInwardKg"))
      || safeNumber(rowValue(row, "RM INWARD KG.", "rmInwardKg"));
    const rmRejectedKg = rawMaterialRejection?.rejectedKg ?? 0;
    const rmUsableKg = Math.max(rmInwardKg - rmRejectedKg, 0);
    const rmPlanningAction = rawMaterialRejection?.planningAction ?? "";
    const rejectionBalanceRestored = Boolean(rawMaterialRejection && orderKg > 0 && rmUsableKg >= orderKg);
    const originalRmInwardDate = rowText(rmInward ?? {}, "RM I/W DATE", "rmInwardDate")
      || rowText(row, "RM I/W DATE", "rmInwardDate");
    const rmInwardDate = rejectionBalanceRestored
      ? rawMaterialBalanceDate(rmInward, orderKg + rmRejectedKg) || originalRmInwardDate
      : originalRmInwardDate;
    const materialPlanPcs = rawMaterialRejection
      ? rejectionBalanceRestored
        ? orderPcs
        : rmPlanningAction === "continue_accepted_quantity" && orderKg > 0
          ? Math.min(orderPcs, Math.floor(orderPcs * rmUsableKg / orderKg))
          : 0
      : orderPcs;
    const materialPlanningReady = rawMaterialRejection
      ? rejectionBalanceRestored || (rmPlanningAction === "continue_accepted_quantity" && materialPlanPcs > 0)
      : rmReceived;
    const rmReplanFromSetupOne = Boolean(
    ...
    const outputRow = {
      ...
      rmStatus: rawMaterialRejection && !rejectionBalanceRestored
        ? materialPlanningReady ? "Partial RM available" : "Rejected - wait for replacement"
        : rmReceived ? "Received" : "Waiting",
      rmInwardDate,
      rmInwardKg,
      rmRejectedKg: round(rmRejectedKg),
      rmUsableKg: round(rmUsableKg),
      rmPlanningAction,
      rmReplanFromSetupOne,
      materialPlanningReady,
      materialPlanPcs,
...
function projectedRouteDispatchDate({
  ...
  const partCode = rowText(workOrder, "partCode");
  const optionNumber = rowText(workOrder, "optionNumber");
  ...
  for (const [routeIndex, route] of routes.entries()) {
    ...
    const displaySetupNo = setupStepKey(routeSetupNo, optionNumber) || routeSetupNo;
    const cycle = cycleByKey.get(masterKey(route));
    if (!cycle) return physicalPlanEndDate;

    const routePlans = plans.filter((plan) => {
      const plannedSetupNo = rowText(plan, "setupNo", "routeSetupNo");
      return canonicalKey(plannedSetupNo) === canonicalKey(displaySetupNo)
        || canonicalKey(plannedSetupNo) === canonicalKey(routeSetupNo);
    });
    if (routePlans.length) {
      const streams = routePlans.map((plan, index) => ({
        machine: rowText(plan, "machine") || `planned-${routeIndex}-${index}`,
    ...
    if (!routeIndex || !previousCycle || !previousStreams.length) return physicalPlanEndDate;
    const routeMachine = machineMasterFamily(route)
      || rowText(route, "MACHINE USED", "machineUsed", "machine", "M/C NO", "MACHINE NO");
    const machineType = rowText(route, "MACHINE TYPE", "machineType");
    ...
    const machineCount = requiredMachineCountForTarget({
      ...
      readyDate,
      deadlineDate,
      machineNextSetupDate: new Map<string, string>(),
      planningCalendar,
    });
...
function qualityParameterMasterEntryKey(row: Record<string, unknown>) {
  return [
    rowText(row, "partNo", "partCode", "PART NO", "PART CODE"),
    rowText(row, "optionNumber", "OPTION NUMBER", "OPTION NO"),
    rowText(row, "setupNo", "SETUP NO.", "SETUP NO", "SET UP"),
    rowText(row, "code", "parameterCode", "CODE"),
  ].map(canonicalKey).join("|");
}

function hourlyQualityCheckEntryKey(row: Record<string, unknown>) {
  return rowText(row, "checkId") || [
    rowText(row, "prodDate", "date"),
    rowText(row, "shift"),
    rowText(row, "hourSlot"),
    rowText(row, "machine", "machineNo"),
    rowText(row, "partCode", "partNo"),
...
function combinedRows(workOrderRows: Array<Record<string, unknown>>, rawByJc: Map<string, { outputQty: number }>, routeGroups: Map<string, Record<string, unknown>[]>, cycleKeys: Set<string>, toolingKeys: Set<string>) {
  ...
  for (const row of workOrderRows) {
    ...
    group.orders += 1;
    if (isMaterialPlanningReady(row)) group.rmReady += 1;
  ...
  for (const row of rows) {
    ...
    if (!current) {
      byJc.set(key, {
        ...
        rmReceipts: [{ receivedDate, receivedKg }],
      });
      continue;
    }
    ...
    const receipts = Array.isArray(current.rmReceipts) ? current.rmReceipts : [];
    current.rmReceipts = [...receipts, { receivedDate, receivedKg }];
  }
  return byJc;
}

function rawMaterialBalanceDate(
  rmInward: Record<string, unknown> | undefined,
  requiredReceivedKg: number,
) {
  if (!rmInward || requiredReceivedKg <= 0 || !Array.isArray(rmInward.rmReceipts)) return "";
  let receivedKg = 0;
  for (const receipt of [...rmInward.rmReceipts]
    .map(asRecord)
    .sort((a, b) => rowText(a, "receivedDate").localeCompare(rowText(b, "receivedDate")))) {
    receivedKg += safeNumber(rowValue(receipt, "receivedKg"));
    if (receivedKg + 0.00000001 >= requiredReceivedKg) return rowText(receipt, "receivedDate");
  }
  return "";
}

function accumulatedRawMaterialRejectionByJobCard(rows: ActionRow[]) {
  const byJc = new Map<string, { rejectedKg: number; planningAction: string; latestCreatedAt: string }>();
  for (const row of rows) {
    if (!isActivePlannerDecision(rowText(row, "status", "STATUS"))) continue;
    const key = canonicalKey(rowText(row, "jcNo", "jobCardNumber", "JC NO.", "JC NO"));
    if (!key) continue;
    const rejectedKg = safeNumber(rowValue(row, "rejectedKg", "REJECTED KG"));
    if (rejectedKg <= 0) continue;
    const createdAt = rowText(row, "createdAt", "occurredAt");
    const planningAction = rowText(row, "planningAction", "PLANNING ACTION");
    const current = byJc.get(key) ?? { rejectedKg: 0, planningAction: "", latestCreatedAt: "" };
    current.rejectedKg += rejectedKg;
    if (!current.latestCreatedAt || createdAt >= current.latestCreatedAt) {
      current.planningAction = planningAction;
      current.latestCreatedAt = createdAt;
    }
    byJc.set(key, current);
  }
  return byJc;
}
...
function machinePlanDetails(
  ...
  rawBySetupAnyMachine: Map<string, PlanningProductionActual & { machines: Set<string> }>,
  routeGroups: Map<string, Record<string, unknown>[]>,
  ...
  for (const row of workOrderRows) {
    ...
    for (const [routeIndex, route] of routes.entries()) {
      ...
      const materialPlanPcsValue = rowValue(row, "materialPlanPcs");
      const materialPlanPcs = materialPlanPcsValue === undefined || materialPlanPcsValue === null || materialPlanPcsValue === ""
        ? customerOrderPcs
        : safeNumber(materialPlanPcsValue);
      const routeChangeQty = remainingQtyBySetup.get(canonicalKey(displaySetupNo));
      const requestedGoodQty = routeChangeQty === undefined
        ? materialPlanPcs
        : safeNumber(rowValue(row, "rmRejectedKg")) > 0
          ? Math.min(routeChangeQty, materialPlanPcs)
          : routeChangeQty;
      ...
      const rmReplanRequired = routeIndex === 0 && rowValue(row, "rmReplanFromSetupOne") === true;
      ...
      const planOverrideInterruption = override ? planOverrideInterruptionForSetup(override, {
        jcNo: rowText(row, "jcNo"),
        setupNo: displaySetupNo,
        lockedMachines: interruptedLockedMachines,
      }) : undefined;
      ...
      const planOverridePlacement = override ? planOverrideQueuePlacementForSetup(override, {
        jcNo: rowText(row, "jcNo"),
        partCode,
        setupNo: displaySetupNo,
      }) : undefined;
      ...
      const unavailableSplitPlan = machineUnavailableSplitPlan({
        setupOrderPcs,
        assignedMachines,
      ...
      for (const assignment of machineAssignments) {
        ...
        let productionActual = rawBySetup.get(productionSetupKey({
          jcNo: rowText(row, "jcNo"),
          partCode,
          setupNo: displaySetupNo,
          machine,
        ...
        rmInwardKg: safeNumber(rowValue(row, "rmInwardKg")),
        rmRejectedKg: safeNumber(rowValue(row, "rmRejectedKg")),
        rmUsableKg: safeNumber(rowValue(row, "rmUsableKg")),
        rmPlanningAction: rowText(row, "rmPlanningAction"),
        rmReplanRequired,
        rmReplanDate: rmReplanRequired ? dateLabel(plannedStartDate) : "",
        ...
        pendingGoodQty: Math.max(machineOrderPcs - (productionActual?.actualQty ?? 0), 0),
        rawRejectQty: Math.max((productionActual?.outputQty ?? 0) - (productionActual?.actualQty ?? 0), 0),
      ...
      const bufferArgs = {
        productionStreams: routeProductionStreams,
        orderPcs: setupOrderPcs,
        previousCycle: cycle,
        nextCycle,
        nextMachineCount: 1,
...
function settleMachineAndSetupSchedule(
  ...
  for (let iteration = 0; iteration < maximumGapBalancePasses; iteration += 1) {
    ...
    const balancedMachine = balanceMachineFamilyIdleGaps(details, planningCalendar, machineRows, lastBalancedMachine, machineUnavailableWindows);
    if (balancedMachine) {
...
function balanceMachineFamilyIdleGaps(details: Array<Record<string, unknown>>, planningCalendar: PlanningCalendar, masterRows: Array<Record<string, unknown>>, afterMachine: string, machineUnavailableWindows: MachineUnavailableWindow[]) {
  const byMachine = new Map<string, Array<Record<string, unknown>>>();
  for (const master of masterRows) {
    const machine = rowText(master, "machine", "machineNo", "MACHINE NO", "M/C NO", "MACHINE NO.");
    if (machine) byMachine.set(machine, []);
  }
  ...
  for (const [machine, machineRows] of machines) {
    ...
    const canUseGap = (row: Record<string, unknown>, start: string) => {
      if (!targetWindows.length) return true;
      const duration = plannedProductionDays(parseDate(rowText(row, "plannedProductionStartDate")), parseDate(rowText(row, "plannedProductionEndDate")), planningCalendar);
      const end = maxDateValue(addDays(start, Math.max(0, duration - 1), planningCalendar), planningMeta(row).minimumProductionEndDate ?? "");
      return !firstOverlappingMachineUnavailableWindow(targetWindows, start, end);
    };
    ...
    for (let index = 0; index < sorted.length - 1; index += 1) {
      ...
      const candidate = familyIdleGapCandidate(details, {
        ...
        planningCalendar,
      }, candidateRows.filter(row => canUseGap(row, gapStart)));
      if (!candidate) continue;

      moveToFamilyIdleGap(candidate, machine, gapStart, gapEnd, machineUnavailableWindows);
      return machine;
    }
    // The last reservation (or an empty queue) has no next row to delimit a gap.
    ...
    const tailCandidate = candidateRows
      ...
      .sort((a, b) => safeNumber(b.plannerPriorityScore) - safeNumber(a.plannerPriorityScore)
        ...
        || rowText(a, "jcNo").localeCompare(rowText(b, "jcNo"), undefined, { numeric: true }))[0];
    if (tailCandidate) {
      tailMoves.push({ row: tailCandidate, machine,
        start: maxDateValue(tailStart, queueReadyDate(tailCandidate)),
        end: addDays(parseDate(rowText(tailCandidate, "plannedProductionStartDate")), -1, planningCalendar),
      });
    }
  }
  const earliestTailMove = tailMoves.sort((a, b) =>
    safeNumber(b.row.plannerPriorityScore) - safeNumber(a.row.plannerPriorityScore)
    || a.start.localeCompare(b.start)
    || queueReadyDate(a.row).localeCompare(queueReadyDate(b.row))
    || rowText(a.row, "jcNo").localeCompare(rowText(b.row, "jcNo"), undefined, { numeric: true }))[0];
  if (earliestTailMove) {
    const { row, machine, start, end } = earliestTailMove;
    moveToFamilyIdleGap(row, machine, start, end, machineUnavailableWindows);
    return machine;
  }
  return "";
}

function moveToFamilyIdleGap(row: Record<string, unknown>, machine: string, start: string, end: string, windows: MachineUnavailableWindow[]) {
  const fromMachine = rowText(row, "machine");
  row.familyIdleGapOriginalEnd = parseDate(rowText(row, "plannedProductionEndDate"));
  row.machine = machine;
  planningMeta(row).machineUnavailableWindows = machineUnavailableWindowsFor(windows, machine);
  row.machineAssignment = "Family idle gap balance";
  row.familyIdleGapFromMachine = fromMachine;
  row.familyIdleGapTargetStart = start;
  row.familyIdleGapTargetEnd = end;
...
function revertInvalidFamilyIdleGapMoves(
  ...
  for (const row of details) {
    ...
    const originalEnd = parseDate(rowText(row, "familyIdleGapOriginalEnd"));
    if (finalEnd <= targetEnd && (!originalEnd || finalEnd < originalEnd) && !overlapsUnavailableWindow) continue;
    const fromMachine = rowText(row, "familyIdleGapFromMachine");
    if (!fromMachine) continue;
    row.machine = fromMachine;
    planningMeta(row).machineUnavailableWindows = machineUnavailableWindowsFor(machineUnavailableWindows, fromMachine);
  ...
  for (const rows of grouped.values()) {
    ...
    for (const [index, group] of setupGroups.entries()) {
      ...
      const bufferReadyDate = nextGroup ? plannedWipBufferReadyDate({
        ...
        nextCycle,
        nextMachineCount: Math.max(1, nextGroup.rows.length),
        actuals,
        planningCalendar,
      }) : "";
...
function rescheduleMachineQueues(details: Array<Record<string, unknown>>, planningCalendar: PlanningCalendar) {
  ...
  while (queues.some(item => item.queue.length)) {
    ...
    let machineNextDate = item.nextDate;
    {
      ...
      const rmReplanRequired = rowValue(row, "rmReplanRequired") === true;
      const planningActualStartDate = rmReplanRequired ? "" : productionActualStartDate;
      ...
      const unavailableDelay = machineUnavailableProductionDelay(row, plannedProductionStartDate, plannedProductionEndDate, planningCalendar);
      if (!planningActualStartDate && unavailableDelay.productionStartDate) {
        plannedProductionStartDate = unavailableDelay.productionStartDate;
        if (machineUnavailableCanMoveSetupDate(row)) plannedStartDate = maxDateValue(plannedStartDate, plannedProductionStartDate);
        plannedProductionEndDate = maxDateValue(
          plannedProductionEnd(plannedProductionStartDate, meta.orderPcs ?? 0, meta.cycle, meta.productionActual, planningCalendar),
          meta.minimumProductionEndDate ?? "",
          liveRunningMinimumEndDate(row, planningCalendar),
        );
      } else if (planningActualStartDate && unavailableDelay.window) {
        plannedProductionEndDate = maxDateValue(
          plannedProductionEndDate,
          machineUnavailableRemainingProductionEnd(row, unavailableDelay.window, planningCalendar),
        );
      }
      if (shopFloorRowIsComplete(row)) {
        plannedProductionEndDate = parseDate(rowText(row, "actualCompletionDate", "shopFloorUpdatedAt")) || plannedProductionEndDate;
      }
      if (unavailableDelay.window) {
        row.machineUnavailableReason = machineUnavailableMessage(unavailableDelay.window);
        row.plannerActionRequired = uniqueTextValues([rowText(row, "plannerActionRequired"), rowText(row, "machineUnavailableReason")]).join("; ");
      }
...
function activeMachineUnavailableWindows(machineConstraints: ActionRow[]): MachineUnavailableWindow[] {
  ...
  return machineConstraints
    ...
    .sort((left, right) => left.fromDate.localeCompare(right.fromDate) || left.machine.localeCompare(right.machine, undefined, { numeric: true }));
}

function machineUnavailableInterruptedSetups(row: Record<string, unknown>): MachineUnavailableInterruption[] {
  const raw = row.interruptedSetups;
  if (!Array.isArray(raw)) return [];
  return raw
    .filter((item) => typeof item === "object" && item !== null && !Array.isArray(item))
    .map((item) => item as Record<string, unknown>)
...
function candidateMachineAvailableForAssignment(
  ...
  const windows = machineUnavailableWindowsFor(context.machineUnavailableWindows, machine).filter(machineUnavailableWindowWantsAlternate);
  if (!windows.length) return true;
  const machineReadyDate = context.machineNextSetupDate.get(canonicalKey(machine)) ?? "";
  const startDate = maxDateValue(context.readyDate, machineReadyDate);
  const endDate = plannedProductionEnd(startDate, context.orderPcs, context.cycle, undefined, context.planningCalendar);
  return !firstOverlappingMachineUnavailableWindow(windows, startDate, endDate);
}
...
function machineUnavailableProductionDelay(
  ...
  planningCalendar: PlanningCalendar,
): { productionStartDate?: string; window?: MachineUnavailableWindow } {
  const meta = planningMeta(row);
  const windows = meta.machineUnavailableWindows ?? [];
  if (!windows.length) return {};
  let adjustedStartDate = parseDate(productionStartDate) || productionStartDate;
  let adjustedEndDate = parseDate(productionEndDate) || productionEndDate;
  if (!adjustedStartDate || !adjustedEndDate) return {};
  let matchedWindow: MachineUnavailableWindow | undefined;
  ...
  for (let guard = 0; guard < 20; guard += 1) {
    ...
    if (!adjustedEndDate) break;
  }

  return matchedWindow && adjustedStartDate !== (parseDate(productionStartDate) || productionStartDate)
    ? { productionStartDate: adjustedStartDate, window: matchedWindow }
    : matchedWindow
      ? { window: matchedWindow }
      : {};
}

function machineUnavailableRemainingProductionEnd(
  row: Record<string, unknown>,
  window: MachineUnavailableWindow,
  planningCalendar: PlanningCalendar,
) {
  ...
  return plannedProductionEnd(resumeDate, remainingQty, meta.cycle, undefined, planningCalendar);
}
function machineUnavailableSplitPlan({
  setupOrderPcs,
  assignedMachines,
  interruption,
  planningCalendar,
...
function mergedProductionActual(
  actual: PlanningProductionActual | undefined,
  manualActual: PlanningProductionActual | undefined,
) {
  if (!manualActual) return actual;
  if (!actual) return manualActual;
  const dates = new Set([...actual.dates, ...manualActual.dates]);
  return {
    ...actual,
    startDate: actual.startDate || manualActual.startDate,
    latestDate: maxDateValue(actual.latestDate, manualActual.latestDate),
    outputQty: Math.max(actual.outputQty, manualActual.outputQty),
    actualQty: Math.max(actual.actualQty, manualActual.actualQty),
    rows: Math.max(actual.rows, manualActual.rows),
    dates,
  };
}

function firstOverlappingMachineUnavailableWindow(windows: MachineUnavailableWindow[], startDate: string, endDate: string) {
...
function machineUnavailableCanMoveSetupDate(row: Record<string, unknown>) {
  return priorityQueueState(row) === "idle";
}

function machineUnavailableMessage(window: MachineUnavailableWindow) {
...
function planOverrideInterruptionForSetup(
  ...
  return {
    ...
    finishedQty: Math.max(0, match.finishedQty),
    window: {
      machine: canonicalKey(lockedMachine),
      fromDate,
      toDate: fromDate,
      action: "shift_required",
      reason: rowText(override, "reason", "REASON") || "Planner machine switch",
      interruptedSetups: interruptions,
      queuePlacements: machineUnavailableQueuePlacements(override),
    },
  };
}

function planOverrideQueuePlacementForSetup(
  override: ActionRow,
  target: { jcNo: string; partCode: string; setupNo: string },
): MachineUnavailableQueuePlacement | undefined {
  if (!isActivePlannerDecision(rowText(override, "status", "STATUS"))) return undefined;
  ...
  return machineUnavailableQueuePlacementForSetup([{
    ...
    toDate: today,
    action: "shift_required",
    reason: rowText(override, "reason", "REASON"),
    interruptedSetups: machineUnavailableInterruptedSetups(override),
    queuePlacements: machineUnavailableQueuePlacements(override),
  }], target);
}
...
function assignedPhysicalMachines({
  ...
  const candidates = candidatePhysicalMachines(routeMachine, machineType, machineRows, machineLoad, machineNextSetupDate, machinePlannedDays, machinePlannedQty);
  const availableCandidates = candidates.filter((candidate) => candidateMachineAvailableForAssignment(candidate.machine, {
    readyDate,
    orderPcs,
    cycle,
    machineNextSetupDate,
...
function plannedWipBufferReadyDate({
  ...
  productionStreams: WipProductionStream[];
  orderPcs: number;
  ...
  const actualStreams = uniqueProductionActuals(actuals)
    .map((actual) => {
      const quantity = Math.max(actual.actualQty, 0);
...
function minDateValue(...values: string[]) {
  return values.map(parseDate).filter(Boolean).sort()[0] ?? "";
}
