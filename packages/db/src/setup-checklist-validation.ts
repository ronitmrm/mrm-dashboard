...
export async function assertSettingChecklistComplete(
  ...
  const items = await client.query<{ prompt: string; section: string; satisfied: boolean }>(
    `SELECT item.prompt, COALESCE(item.source_payload->>'section', '') AS section,
       CASE WHEN lower(btrim(item.response_type)) = 'checkbox'
         THEN answer.response_boolean IS TRUE
           OR lower(btrim(answer.response_text)) IN ('yes', 'true', '1', 'ok', 'pass', 'passed')
         ELSE answer.response_boolean IS NOT NULL OR answer.response_numeric IS NOT NULL
           OR NULLIF(btrim(answer.response_text), '') IS NOT NULL
       END AS satisfied
