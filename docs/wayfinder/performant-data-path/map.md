...
- Dashboard delivery retains same-floor content through checks and retryable failures, distinguishes stale/refresh/error/coverage states, clears state on floor change, checks canonical state every 60 seconds while visible, and schedules no periodic hidden-tab requests.
