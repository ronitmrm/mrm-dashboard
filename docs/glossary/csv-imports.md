...
CSV bulk assignment supports only `Appointed` and `Joined`. Occupied posts must
be vacated manually before assignment; the repository rechecks vacancy at upload
time so an older downloaded file cannot overwrite a later assignment. The CSV
does not include a last-working-date field.
